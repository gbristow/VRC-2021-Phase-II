...
class T265(object):
    ...
    def get_rs_devices(self) -> Dict:
        ...
        rs_devices = {}
        rs_context = rs.context()  # type: ignore
        for i in range(len(rs_context.devices)):
            rs_device = rs_context.devices[i]
            rs_device_name = rs_device.get_info(rs.camera_info.name)  # type: ignore
            rs_device_sid = rs_device.get_info(rs.camera_info.serial_number)  # type: ignore
    ...
    def get_pipe_data(self):
        ...
        frames = self.pipe.wait_for_frames()  # type: ignore
