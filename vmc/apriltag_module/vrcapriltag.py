...
try:
    from apriltag_library import AprilTagVPS  # type: ignore
except ImportError:
...
class VRCAprilTag(object):
    def __init__(self):
        ...
        self.topics = None
        self.at: Optional[AprilTagVPS] = None  # AprilTag object
        self.tm = {}
    ...
    def setup_transforms(self):
        ...
        H_cam_aeroBody = t3d.affines.compose(
            self.default_config["cam"]["pos"], rmat, [1, 1, 1]
        )

        H_aeroBody_cam = np.linalg.inv(H_cam_aeroBody)
        self.tm["H_aeroBody_cam"] = H_aeroBody_cam

        for tag in self.default_config["tag_truth"]:
            name = "tag" + tag
            tag_dat = self.default_config["tag_truth"][tag]
            rmat = t3d.euler.euler2mat(
                tag_dat["rpy"][0], tag_dat["rpy"][1], tag_dat["rpy"][2], axes="rxyz"
            )
            tag_tf = t3d.affines.compose(tag_dat["xyz"], rmat, [1, 1, 1])
    ...
    def handle_tag(self, tag):
        ...
        if str(tag.tag_id) not in self.default_config["tag_truth"].keys():
    ...
    def loop(self):
        ...
        while True:
            ...
            if (
                ...
                and current_tags
                and (current_timestamp != prev_timestamp)
            ):
                ...
                for tag in current_tags:
                    ...
                    if position is not None and error < best_error:
                        best_tag_id = tag_id
    ...
    def main(self):
        ...
        for thread in threads:
            thread.start()
            logger.debug(f"AT: starting thread: {thread.name}")  # type: ignore
